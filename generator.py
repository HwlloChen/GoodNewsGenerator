...
class NewsGenerator:
    ...
    def __init__(self, assets_dir: str = None):
        ...
        self.emoji_font_path = os.path.join(
            self.assets_dir, 'NotoColorEmoji.ttf')
    ...
    def _calculate_font_size(self, text: str, max_width: int, max_height: int,
        ...
        def get_text_width(text: str, font_size: int) -> float:
            ...
            if has_emoji and PILMOJI_AVAILABLE:
                width, _ = self._get_text_size_pilmoji(
        ...
        while current_line_count <= 3:
            ...
            chars_per_line = (
                len(text) + current_line_count - 1) // current_line_count
            ...
            while font_size >= min_font_size:
                ...
                for cpl_adjust in [0, -1, 1, -2, 2]:
                    ...
                    lines = textwrap.wrap(
                        text, width=cpl, break_long_words=True, break_on_hyphens=False)
    ...
    def _is_emoji(self, char: str) -> bool:
        ...
        判断单个字符是否为Emoji
        """
        return unicodedata.category(char) == 'So' or '\U0001F000' <= char <= '\U0001F9FF'

    def _draw_text_with_emoji_margin(self, img: Image.Image, text: str, x: float, y: float,
                                     font_size: int, font_color: tuple, stroke_width: int = 0,
                                     stroke_color: tuple = None, font_path: str = None, emoji_margin: int = 8):
        """
        按字符绘制文本，遇到emoji字符时左右加边距
        """
        if font_path and os.path.exists(font_path):
            main_font = ImageFont.truetype(font_path, font_size)
        else:
            ...
            with Pilmoji(img, emoji_scale_factor=1.3) as pilmoji:
                for char in text:
                    if self._is_emoji(char):
                        cur_x += emoji_margin  # 左边距
                        pilmoji.text((int(cur_x), int(y)), char, fill=font_color,
                                     font=main_font, stroke_width=stroke_width, stroke_fill=stroke_color)
                        # 计算emoji宽度
                        char_width, _ = pilmoji.getsize(char, font=main_font)
                        cur_x += char_width + emoji_margin  # 右边距
                    else:
                        # 普通字符
                        draw.text((cur_x, y), char, fill=font_color, font=main_font,
                                  stroke_width=stroke_width, stroke_fill=stroke_color)
                        char_width = main_font.getlength(char)
                        cur_x += char_width
        else:
            # pilmoji不可用时，全部用Pillow绘制
            for char in text:
                if self._is_emoji(char):
                    cur_x += emoji_margin
                draw.text((cur_x, y), char, fill=font_color, font=main_font,
    ...
    def _draw_text_with_emoji(self, img: Image.Image, text: str, x: float, y: float,
        ...
        """
        绘制支持Emoji的文本（带emoji边距）
        """
        if PILMOJI_AVAILABLE and self._has_emoji(text):
            # 使用自定义方法绘制，带emoji边距
            self._draw_text_with_emoji_margin(
                img, text, x, y, font_size, font_color, stroke_width, stroke_color, font_path, emoji_margin=8
            )
        else:
            # 使用传统方式绘制文本
            draw = ImageDraw.Draw(img)
    ...
    def generate(self,
        ...
        # 打开背景图片
        try:
            img = Image.open(template['image']).convert(
                'RGBA')  # 转换为RGBA以支持透明度
        ...
        for line in lines:
            ...
            if self._has_emoji(line) and PILMOJI_AVAILABLE:
                line_width, _ = self._get_text_size_pilmoji(
                    line, font_size, font_path)
            else:
                font = self._get_font(font_size, font_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--type", choices=["good", "bad"], default="good", help="图片类型，good表示喜报，bad表示悲报")
    ...
    generator.generate(args.text, news_type=args.type,
                       output_path=args.output, font_path=args.font)
